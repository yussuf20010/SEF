...
export class CoreCoursesCourseListItemComponent implements OnInit, OnDestroy, OnChanges {
    ...
    action(action: string, event: MouseEvent): void {
        event.stopPropagation(); // Prevents the card click from triggering
        if (action === 'favourite') {
            this.course.isfavourite = true;
        } else if (action === 'unfavourite') {
            this.course.isfavourite = false;
        }
    }

    detailsHandler(event: MouseEvent): void {
    ...
    handleButtonClick(event: MouseEvent): void {
        ...
        this.openCourse(); // Your logic for the details button
    }

    handleFavouriteClick(action: string, event: MouseEvent): void {
        event.stopPropagation(); // Prevent the card's click event from triggering
        if (action === 'favourite') {
            this.course.isfavourite = true;
        } else if (action === 'unfavourite') {
